...
class VlImagePreprocessor(core.IImagePreprocessor):
    def preprocess(self, img: core.FileSysImage):
        return img.to_pil()


class VlPromptFormatter(core.IPromptFormatter):
    def format(self, ques: core.Question) -> str:
        return ques.text


class VilaVLM(core.BaseVLM):
    ...
    def init_model(self):
        ...
        _model_path = f'Efficient-Large-Model/{self.model_name}'
        ...
        self.tokenizer, self.model, self.image_processor, self.context_len = (
            load_pretrained_model(_model_path, _model_name, model_base=self.model_base,
...
class BasicTextExtractor(core.BaseExtractor):  # TODO tokenizer extractor
...
def main():
    ...
    llava_vlm = VilaVLM(
        ...
        formatter=VlPromptFormatter(),
        image_preprocessor=VlImagePreprocessor())
...
# the original code for VILA1.5-7b does not run.
# need to change line 151 in llava/model/builder.py by changing kwargs to device_map='auto'


DEBUG=1 python3 main.py --model_name VILA1.5-3b
"""
